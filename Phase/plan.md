...
- **3.4.3. 現状の知見と課題 (Findings)** [New]:
    - **コントラストの影響**: 背景（机など）とのコントラストが弱い場合、書類の輪郭認識が不安定になる。
    - **黒マットの効果**: 黒いマットを敷くことで、白い書類とのコントラストが明確になり、認識精度が劇的に向上することを確認。
    - **書類内容の読み取り**: 現在、患者ID（診察券）の抽出は実装・検証済みだが、書類本体のテキスト内容の読み取りについては**未実装ないし未検証**。
...
- [ ] **3.2.3. ステータス表示の改善**
    ...
    - 最終スキャン時刻の表示
- [ ] **3.2.4. プレビューウィンドウの実装**
    - 「Show Preview」メニューの実装
    - `cv2.imshow` を使用したリアルタイム映像表示
    - 動体検知エリアのオーバーレイ表示 (デバッグ用)
